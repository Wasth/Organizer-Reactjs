class Task extends React.Component {
    ...
    save(e) {
        this.setState({
            edit:false,
            text: this.refs.newvalue.value
        });
        this.props.onsave(this.props.i, this.refs.newvalue.value);
    }
    check() {
        this.setState({
                ...
                return (
                    <div className="task list-group-item row">
                        <div className="row">
                            ...
                            <h5 className='task-text mt-2 col' style={this.state.checked ? {'textDecoration': 'line-through'}:{}}>{this.props.text}</h5>
...
class Taskerold extends React.Component {
    ...
    render () {
        const mytasks = this.state.tasks.map((task, i) => {
            return (<Task key={i} text={task} i={i} ondel={this.deleteTask.bind(this)} onsave={this.saveTask.bind(this)} />)
        });
        return (
